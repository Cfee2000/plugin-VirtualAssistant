...
/**
 * This Functional Component renders the AI-generated Summarization specifically of the Dialogflow CX Virtual Agent conversation with the customer.
 * @param {*} props 
 * @returns 
 */
const VoiceCallSummary = (props) => {
		...
		const retrieveSummary = async () => {
			...
			try {
				...
				while (
					...
					console.log(
						"OPENAI API CALL IS NOT RETURNING DATA FOR THE VOICE CALL SUMMARY RIGHT NOW. INITIATING RETRY LOGIC"
					);
				...
				if (data.choices[0].text) {
					...
					throw new Error(
						"Failed to get voice call summary from OpenAI after retries"
					);
				}
			} catch (error) {
				/* show error message */
					...
					<Stack orientation="vertical" spacing="space60">
						<Stack orientation="horizontal" spacing="space50">
							...
							<Heading as="h3" variant="heading30" marginBottom="space0">
								Summary of Virtual Agent Interaction
							</Heading>
						</Stack>
						<Card>
							{editSummary ? (
...
export default React.memo(VoiceCallSummary);
